...
# function for sending messages about the need to perform activation
def send_activation_notification(user):
    ...
    user.email_user(subject, body_text)  # отпраивть зарег пользоваьелю


# function that generates the names of the uploaded files stored in the model
def get_timestamp_path(instance, filename):
    return '%s%s' % (datetime.now().timestamp(), splitext(filename)[1])


# function that sends notifications about new comments
